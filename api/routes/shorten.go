...
func ShortenURL(c *fiber.Ctx) error {
	...
	if err == redis.Nil {
		rate_limit_time, _ := strconv.Atoi(os.Getenv("RATE_LIMIT_TIME"))
		_ = r2.Set(database.Ctx, c.IP(), os.Getenv("API_QUOTA"), time.Duration(rate_limit_time)*60*time.Second).Err()
	} else {
